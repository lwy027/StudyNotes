...
export default class BSTree<T> extends TreeNode<T> {
  ...
  insert(value: T) {
    //1.常见新节点
    const newTreeNode = this.creatNode(value);
...
◼ 2> 找到要删除节点
  ...
  remove(value: T): boolean {
    ...
    //定义删除节点
    const delNode: TreeNode<T> = current;
    ...
    if (this.root && current.parent) {
      this.root.parent = current.parent;
    }
